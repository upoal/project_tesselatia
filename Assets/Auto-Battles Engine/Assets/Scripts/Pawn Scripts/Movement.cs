...
{    
    public class Movement : MonoBehaviour
    {
        ...
        #region Movement Fix 1/6/2025
        //Movement fix 1/6/2025
        Coroutine clearPreviousTilesRoutine;

        private IEnumerator ClearPreviousTilesAfterSeconds(float seconds)
        {
            float timeElapsed = 0;
            ...
            clearPreviousTilesRoutine = null;
        }
        ...
        {
            if (clearPreviousTilesRoutine != null)
            {
                StopCoroutine(clearPreviousTilesRoutine);
            }
        }
        #endregion
        ...
        public virtual void SetCurrentTileInCombat(ChessBoardTile tile)
        {
            ...
            //start timer to reset our previous tiles after X seconds
            //This will avoid our pawn getting stuck 
            
            //if our previous tiles clear timer is already running (coroutine), cancel it first and then restart it
            if (clearPreviousTilesRoutine != null)
            {
                StopCoroutine(clearPreviousTilesRoutine);
            }

            //start timer to clear previous tiles
            StartCoroutine(ClearPreviousTilesAfterSeconds(3));
